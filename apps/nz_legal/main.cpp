...
//   POST /notable     → Qdrant payload filters for notable cases
//   POST /sentencing-tracker → Qdrant payload filters for sentencing cases
//   POST /pg-tracker  → Qdrant payload filters for personal grievance cases
...
//        -d '{"question":"What did the Court of Appeal say about s 27 NZBORA?"}'
//   curl -N -X POST http://localhost:8080/ask/stream -H 'Content-Type: application/json'
//        -d '{"question":"Find recent Supreme Court cases on tikanga"}'
...
namespace astraea::detail::nz_legal_app {

/// @brief Deserialized body of a POST /ask, POST /ask/stream, or POST /zone request.
///
...
struct AskRequest {
    ...
    /// @brief Debug mode key; enables debug/context_debug/debug_done SSE events when equal to DEBUG_KEY env var.
    std::string debug_key;
    /// @brief When true, emit context_debug without requiring debug_key (used by the feedback capture UI).
    bool feedback_context = false;
    /// @brief Optional NZ property address; resolved to district plan zone info prepended to the question.
    std::string address;
};
...
struct ParsedAskRequest {
    ...
    std::string address; ///< Forwarded from AskRequest.
};
...
/// from the payload (e.g. "NZLEG/BuildingAct2004/s7").
struct SourceJson {
    std::string case_id;    ///< Case or legislation reference from the Qdrant payload, not the internal UUID.
    std::string court_name; ///< Act or authority name, used by renderSources to build the link label.
    std::string date;       ///< Date string (e.g. year of enactment), used by renderSources.
    std::string url;        ///< Public legislation or decision URL; renderSources falls back to "#" if not https://.
};

} // namespace astraea::detail::nz_legal_app

template <>
struct glz::meta<astraea::detail::nz_legal_app::SourceJson> {
    using T = astraea::detail::nz_legal_app::SourceJson;
...
struct LegSourceJson {
    std::string case_id; ///< Legislation chunk ID, e.g. "NZLEG/BuildingAct2004/s7".
...
struct SourcesEvent {
    ...
    std::vector<SourceJson>    sources; ///< Case-law or legislation citations for this response.
...
struct VerificationSection {
    ...
    std::string reference; ///< Human-readable section reference, e.g. "s 7 Building Act 2004".
...
struct ConfidenceEvent {
    ...
    int         chunks = 0; ///< Total case-law or legislation chunks retrieved.
...
struct DebugEvent {
    ...
    int                chunks      = 0; ///< Total chunks retrieved.
...
struct ChunkCard {
    ...
    std::string date; ///< Date from the Qdrant payload.
    int         tokens       = 0; ///< Approximate token count (chars / 4).
...
struct ContextBudget {
    ...
    int sources_sent    = 0; ///< Number of sources included in the LLM prompt.
...
/// @brief POST /zone request body.
struct ZoneApiRequest {
    std::string address; ///< NZ property address to look up (plain text; geocoded by the sidecar).
};

/// @brief Nested zone object in the /zone response.
///
/// Matches Python lookup_zone() output dict fields so the frontend can read
/// data.zone.zone_name and data.zone.zone_code without changes.
struct ZoneObj {
    std::string zone_code; ///< Short district plan zone code, e.g. "MHU".
    std::string zone_name; ///< Human-readable zone name, e.g. "Mixed Housing Urban".
    std::string council; ///< Council name that administers this zone, e.g. "Auckland Council".
};

/// @brief POST /zone JSON response body.
///
/// Shape matches Python jurisdiction.py register_routes() /zone endpoint so
/// the existing frontend (data.zone.zone_name) works without changes.
struct ZoneApiResponse {
    bool        found   = false; ///< True when the address was geocoded and a zone was found.
    std::string address; ///< Echo of the input address after normalisation.
    double      lat     = 0.0; ///< Latitude of the resolved property; 0.0 when not found.
    double      lng     = 0.0; ///< Longitude of the resolved property; 0.0 when not found.
    std::optional<ZoneObj> zone; ///< Zone details; absent when found is false.
    std::string error; ///< Error message; empty string on success.
};
...
struct RouteDebugEntry {
    ...
    std::vector<LogSource>   sources; ///< Retrieved sources (UUID + score for internal logging).
    ...
    int                      context_chunks = 0; ///< Total chunks (sources + legislation + guidance) in the prompt.
...
struct HealthResponse {
    ...
    std::string jurisdiction; ///< The jurisdiction name, e.g. "nz-legal".
};
...
using namespace astraea::detail::nz_legal_app;
...
int main() {
    ...
    // corpus. A missing section silently no-ops at runtime — see the
    // identical block in apps/nz_tenancy/main.cpp for the rationale.
    if (leg_store) {
